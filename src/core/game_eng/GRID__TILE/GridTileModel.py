                ...
                if row == 0 or row == self.height+2 or collumn == 0 or collumn == self.width+2:
                    if row % 2 == 0:
                        ...
                        self.hex_draw_array[row][collumn]=(GridTile(game,
                                                            ...
                                                            random_unit_count,
                                                            (True if row % 2 == 0 else False),
                                                            2 * hex_side + (4 * hex_side) * collumn - (2 * (collumn + 1) - 1) * extra,
                                                            self.sq * hex_side / 2 + (row - 1) * (self.sq * hex_side / 2 + delta) + delta,
                                                            True,
                                                            Color.BLACK)
                                                    )
                else:
    ...
    def inPolygon(self,x, y):
        for row in range(1,self.height+1):
            for collumn in range(1,self.width+1):
                if (y >= self.hex_draw_array[row][collumn].pos_y) and (y<=self.hex_draw_array[row][collumn].pos_y + self.hex_draw_array[row][collumn].sq * self.hex_draw_array[row][collumn].side)\
                        and (x >=self.hex_draw_array[row][collumn].pos_x) and (x <=self.hex_draw_array[row][collumn].pos_x + 1.5 * self.hex_draw_array[row][collumn].side):
