...
class GridModel:
    def __init__(self, game, hex_side=20, width=14, height=5, delta=4):
        ...
        for column in range(self.height):  # нечет-q https://habr.com/ru/post/319644/
            ...
            for row in range(self.width):
                j += 1
                self.add_cell(game, hex_side, row, column, False, i, j)
        self.set_team_starting_zones()

    def add_cell(self, game, hex_side, row, column, is_wall, x, y):
    ...
    def set_team_starting_zones(self):
        ...
        for row in range(6):
            self.set_cell_color_x_y(neighbours[row][0], neighbours[row][1],'gray','gray')
        neighbours = self.get_cell_by_x_y(6, 2).get_neighbours()
        for row in range(6):
            self.set_cell_color_x_y(neighbours[row][0], neighbours[row][1],'dark_orange','dark_orange')
        neighbours = self.get_cell_by_x_y(2, self.height).get_neighbours()
        for row in range(6):
            self.set_cell_color_x_y(neighbours[row][0], neighbours[row][1],'dark_green','dark_green')
    ...
    def attack_cell_units(self, redcell, orangecell, value):
        ...
        if orangecell.value <= 0:
            orangecell.set_team_color_absolute(redcell.return_team_color())
