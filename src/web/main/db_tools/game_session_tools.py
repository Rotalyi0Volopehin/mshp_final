...
class DBGameSessionTools:
    ...
    def try_create_new_session(title: str, turn_period: int, user_limit: int,
        ...
        if not (isinstance(title, str) and isinstance(turn_period, int)
                and isinstance(user_limit, int) and
                isinstance(user_lowest_level, int) and isinstance(user_highest_level, int)):
            raise exceptions.ArgumentTypeException()
        if (turn_period < 0) or (user_limit < 3) or (user_lowest_level < 0)\
                or (user_highest_level < 0):
